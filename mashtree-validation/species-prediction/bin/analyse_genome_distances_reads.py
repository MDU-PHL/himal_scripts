...
Usage:
    python analyse_genome_distances.py <matrix_file> <query_sequence> <output_directory> <seqkit_file>
...
Arguments:
    ...
    seqkit_file: The path to the seqkit file.
...
Example:
    python analyse_genome_distances.py matrix.tab Bacillus_anthracis_HDZK-BYSB7_60 /path/to/output/directory/ seqkit.tab
"""
...
# Function to extract number of sequences from seqkit file
def extract_num_seqs(filename):
    with open(filename, 'r') as file:
        lines = file.readlines()
        for line in lines:
            columns = line.split()
            if columns[0] != 'file':  # Skip the header line
                num_seqs = int(columns[3].replace(',', ''))  # Remove comma and convert to integer
                return num_seqs

# Input file name, query sequence, output directory path, and seqkit file path
matrix_file = sys.argv[1]
query_sequence = sys.argv[2]
output_directory = sys.argv[3]
seqkit_file = sys.argv[4]
seed_genome = '_'.join(query_sequence.split('_')[:-1])

# Extract number of sequences from seqkit file
num_seqs = extract_num_seqs(seqkit_file)
