...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <primer_info> <fasta_directory>"
    echo "++++++++++"
    echo "Make sure to: "
...
mkdir -p pcr_detailed_results

# Get the path to the directory containing the FASTA files
directory="$2"

# Loop over all the FASTA files in the directory
for file in "$directory"/*.fa; do
    ...
    for serotype in "${!primers[@]}"; do
        ...
        output_file="pcr_detailed_results/${sample_id}_${serotype}_pcr.txt"
